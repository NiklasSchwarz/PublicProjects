...
OUTPUT_CHANNELS     = 19                                                        # Hadamard output channels (size of Hadamard codes)
ACCOMULATIVE_COUNTS = 1                                                         # Number of iterations to accumulate gradients
...
MODEL_IN_CHANNELS   = [32, 64, 160, 256]
MODEL_CHANNELS      = 256
EMBED_DIMS          = 32
NUM_LAYERS          = [2, 2, 2, 2]
...
train_dataloader = dict(
    ...
    prefetch_factor=2,                 # pro Worker vorgepufferte Batches (Default=2)
...
model = dict(
    ...
    decode_head=dict(
        ...
        loss_decode=dict(
            ...
            loss_name='loss_ce',
            loss_weight=1.0,
            use_sigmoid=False                                                # Sigmoid for multi-label Hadamard codes
        ),
    ),
